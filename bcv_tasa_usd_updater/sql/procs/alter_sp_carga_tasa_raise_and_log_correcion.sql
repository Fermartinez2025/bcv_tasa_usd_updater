...
BEGIN
    CREATE TABLE dbo.log_sp_errors (
        ...
        sp_name NVARCHAR(128) NOT NULL,
        error_number INT NULL,
        error_message NTEXT NULL, -- Cambiado de NVARCHAR(MAX) a NTEXT
        error_state INT NULL,
        error_severity INT NULL,
        payload NVARCHAR(1000) NULL, 
        created_at DATETIME DEFAULT GETDATE() -- DATETIME2 y SYSUTCDATETIME no existen en 2000
    );
END
GO
...
GO
-----------------------------------Corregido---------------------------------------------------
-- 1. Verificar si existe para eliminarlo y empezar de cero
IF EXISTS (SELECT * FROM sysobjects WHERE id = OBJECT_ID(N'[dbo].[sp_carga_tasa]') AND OBJECTPROPERTY(id, N'IsProcedure') = 1)
    DROP PROCEDURE [dbo].[sp_carga_tasa]
GO

-- 2. Crear el procedimiento desde cero
CREATE PROCEDURE [dbo].[sp_carga_tasa]
    @periodo       NVARCHAR(10),    
    @fechaProceso  NVARCHAR(10),    
    @valor         FLOAT,           
    @valorVenta    FLOAT            
AS
BEGIN
    ...
    -- Conversión de fecha compatible con SQL 2000
    SET @fecha = CAST(@fechaProceso AS DATETIME)
    
    IF @@ERROR <> 0
    BEGIN
        SET @return_code = 1
        RAISERROR('Fecha invalida. Use formato YYYY-MM-DD', 16, 1)
        RETURN @return_code
    END

    -- Lógica de Upsert (SQL 2000 style)
    IF @periodo = 'manana'
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM [dbo].[tasas_dicom] WHERE [fecha] = @fecha)
        BEGIN
    ...
    BEGIN
        ...
        IF EXISTS (SELECT * FROM sysobjects WHERE id = OBJECT_ID(N'[dbo].[log_sp_errors]') AND OBJECTPROPERTY(id, N'IsTable') = 1)
        BEGIN
            INSERT INTO dbo.log_sp_errors (sp_name, error_number, error_message, payload)
            VALUES ('sp_carga_tasa', @err_num, 'Error SQL 2000', @fechaProceso)
        END
        
        RAISERROR('Error en sp_carga_tasa. Codigo: %d', 16, 1, @err_num)
        RETURN @err_num
    END
